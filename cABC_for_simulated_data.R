...
for(i in 1:1000){
  FinalSize[i] <- SellkeEpidemic(2, 200)
}
...
pm3 <- as.data.frame(LambdaTestBins)
colnames(pm3) <- c("M", "lower","lower")
pm4 <- rbind(pm3[,c(1,3)], pm3[,c(1,2)])

ggplot(data = pm4, aes(x=lower, y=M)) + geom_step() + theme_bw()+
  labs(title=expression(paste("Total number infected for each interval of ", lambda)),x=expression(paste(lambda)), y = "Total infected")
...
CoupledSim <- CoupledABC(10000, 200, 1, 167, 2)
